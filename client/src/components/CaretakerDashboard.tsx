...
const CaretakerDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [adherenceRes, activityRes] = await Promise.all([
          api.get("/api/caretaker/adherence", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          api.get("/api/caretaker/recent-activity", {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);

        setAdherenceRate(adherenceRes.data.adherenceRate);
        setCurrentStreak(adherenceRes.data.currentStreak);
        setMissedDoses(adherenceRes.data.missedDoses);

        const activities = activityRes.data.activities || [];
        setRecentActivity(activities);

        const datesTaken = new Set<string>(
          activities.filter(a => a.taken).map(a => a.date)
        );
